...
- [ ] 4.2 サービス別にfetchする関数を実装（`fetch('/data/questions/{service}.json')`）
...
- [ ] 6.1 ローカル開発環境でクイズが正常に表示されることを確認（`npm run dev`）
- [ ] 6.2 ビルド成果物にJSONが含まれないことを確認（`npm run build` 後に `dist/` を確認）
- [ ] 6.3 デプロイ後にCloudFront経由でクイズデータが取得できることを確認
- [ ] 6.4 デプロイ後にS3の `data/questions/` が削除されていないことを確認
- [ ] 6.5 S3 Versioningの確認（Terraformで既設定のため確認のみ）
