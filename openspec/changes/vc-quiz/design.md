...
### 2. S3プレフィックス：`data/vc-quiz/`

**決定**: AWS Quizの `data/questions/` と分離した専用プレフィックスを使用。

**理由**: 将来的な削除・アクセス制御・同期コマンドの独立性を確保するため。同じプレフィックスに混在させると誤削除リスクがある。
...
4. 同一声優の別キャラが正解と同じ作品に出演している場合、その声優を誤答候補から除外
