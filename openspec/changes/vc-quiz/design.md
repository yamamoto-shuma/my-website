...
**決定**: `voice_actors.csv`（声優マスタ）・`titles.csv`（作品マスタ）・`characters.csv`（中間テーブル）の3ファイルCSVで管理。`va_id` / `title_id` で参照。
...
フロント側でtitles + charactersをJOINしてTitle[]型に組み立てる。
